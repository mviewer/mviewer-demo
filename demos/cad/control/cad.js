mviewer.customControls.cad = (function () {
  ...
  var updateIHM = function () {
    ...
    document
      .getElementById("parcelle-select")
      .addEventListener("change", onParcelleChange);
    ...
    if (!document.getElementById("color-picker-btn")) {
      ...
      document
        .querySelectorAll('.mv-layer-details[data-layerid="cad"] .layerdisplay-legend')[0]
        .appendChild(clone);
    }
    ...
    src.forEachFeature(function (feature) {
      if (feature.get("geo_parcelle") === id_parcelle) {
        ...
        let res = mviewer
          .getMap()
          .getView()
          .getResolutionForExtent(feature.getGeometry().getExtent());
        let zoom = mviewer.getMap().getView().getZoomForResolution(res);
        mviewer
          .getMap()
          .getView()
  ...
  return {
    ...
    init: function () {
      if (!_data) {
        fetch("/ressources/demos/demos/cad/data/departements.json").then(function (response) {
