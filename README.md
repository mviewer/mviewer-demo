...
Mviewer v4 ou superieure compatible avec Bootstrap 5.
...
1. Faire un fork de ce dépôt (si j'ai un compte ou organisation `github.com/johndoe` alors je pourrai avoir le fork dans `github.com/johndoe/mviewer-addon`)
2. Faire un clone en local de ce fork pour travailer en local sur votre ordinateur (avec VSCode par exemple)
...
Pour accéder et tester vos démonstrations dans mviewer, vous aurez besoin du coeur du mviewer qui permet le chargement et l'affichage des cartes.
Vous devez donc cloner mviewer sans ses sous-modules (mviewer-demo étant un sous-module de mviewer), modifier l'url du sous-module pour utiliser votre fork, et ensuite récupérez les sources des sous-modules.
...
3. Ouvrez [le fichier .gitmodules](https://github.com/mviewer/mviewer/blob/develop/.gitmodules) qui est à la racine du répertoire `/mviewer` que vous venez de cloner

4. Remplacer l'URL `https://github.com/mviewer/mviewer-demo.git` par l'URL de votre fork (e.g `https://github.com/johndoe/mviewer-demo.git` car j'ai fait un fork dans mon organisation github `johndoe`)
...
Démarrez ensuite votre mviewer (soit via nginx soit via node js), et vous pourrez accéder à toutes les démonstration
